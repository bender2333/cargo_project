...
describe('calculatePacking — 31 Russian pallets in a custom 13400×2450×2650 container', () => {
  it('fits all 31 pallets without violating boundary, overlap, weight, or container limits', () => {
    const fixtureDir = path.resolve(__dirname, '../../test-data/excel')
    const fixtures = fs.readdirSync(fixtureDir).filter((name) => name.endsWith('.xlsx'))
    expect(fixtures.length).toBeGreaterThan(0)

    const buffer = fs.readFileSync(path.join(fixtureDir, fixtures[0]))
