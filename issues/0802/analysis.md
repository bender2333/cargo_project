...
两个提交对应的 focused unit/contract/browser 场景已经由上述证据闭环。其后完整本地 release gate 也已执行：`npm run lint` 通过；`npm test` 通过常规 unit **91 files / 792 tests** 与 packing performance **2 files / 7 tests**；`npm run build` 通过并保留既有 `>500 kB` chunk warning；`npm run test:e2e` **125/125** 通过、零失败/跳过。正式 `npm run benchmark` 首轮因三个 p95 timing gate 返回非零，未修改源码、baseline、threshold、iterations 或 case 后重跑通过且 timing comparable；成功报告为 `test-results/benchmark/frontend-architecture.json`，`jq -e` 再次确认五项 contract hash 完全一致，protected-file diff 为空。首轮失败与重跑实测保留在 `decision.md` 的「本地 benchmark 首轮门禁失败」条目。因此 local release gate 为 GREEN；生产状态见下方部署结果。

### 生产部署结果（已回滚）

生产部署先后创建 backup `/root/cargo_project-backup-20260804-082126` 与 `/root/cargo_project-backup-20260804-084101`，两次部署后 static HTTP 均为 **200**、未认证 API 均为 **401**，本地/远程 static SHA-256 内容一致。第一次完整远程 E2E 为 **124/125**，失败于既有手动历史用例的 React DOM 持续 detach；回滚后该用例单独运行 **1/1**。第二次完整远程 E2E 为 **123/125**，再次出现同一 detach，并新增登录后页面停留在「工作台加载中…」超过 5 秒；回滚后两个失败用例各自单独运行均 **1/1**。两次都没有把失败发布留在生产。

第一次按计划原始 rollback 命令执行时，backup 的 `server/` 不含 SQLite `database.db`，而 `rsync --delete` 删除了 live DB，服务随后重建了 **65536-byte** 新库，SHA-256 `6bb13149dccca51fb34563fc6d184432b3a402c227378f5c7f5aa318bb1b5d73`；该异常在退出非零后立即通过 incident `/root/cargo_project-incident-20260804-163753` 发现，并从其中的 **475136-byte** 数据库恢复，SHA-256 `76c21bbf5c5df7eb05121c9453cfbf6180e8c5dcfbe17a776452077dfae27563`、HTTP/API 健康与 static/backend backup hash 均验证。第二次改用保留 `database.db` 的安全 rollback，incident `/root/cargo_project-incident-20260804-165805` 与 live DB hash 同为 `aba20cc7087c4eefe3579a1b08e1a2421b8c6206dbe0977c362a72d1ef6b53c6`，static/backend 与 backup 完全一致，static **200**、API **401**；最终 `PRAGMA quick_check` 返回 `ok`。当前生产已恢复到本次修复前的已验证 release；0802 修复未留在生产，完整 remote E2E 与 production feature verification 均不为 GREEN。
