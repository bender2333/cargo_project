...
test('H: save-as creates a new template and keeps the original', async ({ page }) => {
  ...
  await expect(page.getByTestId('template-selection-panel').getByRole('button', { name: copyName, exact: true })).toBeVisible()
  await cancelImport(page)
  await deleteTemplatesByName(page, [originalName, copyName])
})
