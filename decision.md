...
## 2026-08-19 Excel 导入与模板配置统一行为设计

- 背景：导入映射、模板保存、单位换算、行号和样例文件测试存在规则分散与静默错误风险；当前审查确认重复映射可生成错误尺寸，模板可保存为不可用状态，组合尺寸没有独立单位，矩阵行号会偏移，`mergeRows: by-label` 没有实际解析语义，样例并发测试仍查找旧 `datalist`。
- 选项：A. 继续在现有组件和解析函数中做局部修补；B. 保留现有 Worker/React/API 架构，建立统一的纯 `Import Analyzer`，集中模板校验、映射、转换、预览和 issue 合同；C. 把预览分析移到服务端并新增上传预览接口。
- 决策：选择 B。任意错误行整批阻止确认；警告可在明确展示后确认；选择模板只预填；组合模式使用独立 `combinedUnit`；保留真实 Excel 源行号；活动合同移除未实现的 `mergeRows`；无标签时允许确定性回退但必须显示警告；源标签超过统一 80 字符上限时显式报错，不静默截断。
- 影响：模板结构有效性与当前文件兼容性分离；模板管理页可保存结构有效但尚未匹配当前样例的配置，导入弹窗仍因缺列阻止；React 不再复制解析规则；有效行与错误行混合时不产生部分导入或当前货物变更。
- 后续：先将行为设计固化为 `docs/superpowers/specs/2026-08-19-import-template-behavior-design.md`，完成用户审阅后再制定实施计划；不得先改实现或放宽现有测试断言。
