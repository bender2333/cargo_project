...
- 旋转待决策：修复前需在“保持原支撑面 z”“围绕几何中心”“旋转后搜索最近合法支撑面”三种语义中确定一种。按真实装柜视角，建议保持原支撑面 z，再由既有边界、重叠和支撑校验决定是否允许；不要放宽 50% 支撑断言。
...
- 模板待确认：用户原文“端口”按上下文暂解释为“入口”。实现前需确认下载通用标准空白模板，还是按已保存映射动态生成客户格式；是否同时提供示例数据。最小默认建议是在“导入 XLSX”旁下载包含当前标准字段的通用空白 `.xlsx`。
