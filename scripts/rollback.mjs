...
export function buildRollbackRemoteScript(config = DEFAULT_CONFIG, backupDir, incidentBase) {
  ...
  return [
    ...
    '}',
    '',
    'for required_command in cmp chmod chown curl find flock mkdir mktemp realpath rsync sha256sum sort sqlite3 stat systemctl timeout dirname basename; do',
    ...
    'if ! static_status="$(curl --connect-timeout 5 --max-time 15 --silent --show-error --output /dev/null --write-out \'%{http_code}\' "$healthcheck_url")"; then',
    '  fatal "Static health request failed: $healthcheck_url"',
    '  exit 1',
    'fi',
    '[ "$static_status" = "200" ] || { fatal "Static health status was $static_status, expected 200"; exit 1; }',
    'if ! api_status="$(curl --connect-timeout 5 --max-time 15 --silent --show-error --output /dev/null --write-out \'%{http_code}\' "$api_healthcheck_url")"; then',
    '  fatal "API health request failed: $api_healthcheck_url"',
    '  exit 1',
    'fi',
    '[ "$api_status" = "401" ] || { fatal "API health status was $api_status, expected 401"; exit 1; }',
    'trap - EXIT',
    'exit 0',
  ].join('\n')
}
