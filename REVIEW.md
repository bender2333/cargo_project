...
- 左侧或顶部显示阶段列表：阶段编号、标签统计、数量、层级、深度区间。
- 右侧复用 2D/3D 展示能力，高亮当前阶段的箱体，并淡化已完成/未开始箱体。
- 提供上一步、下一步、播放/暂停。
- 支持按阶段查看箱体明细。
- 保留标签颜色和标签文字，不能退化成无业务含义的色块动画。
- 后续再接打印/导出：每个阶段一张小图 + 标签数量 + 作业提示。
...
1. 新增 `LoadingTaskGroup` 纯业务生成模块与单元测试，先证明阶段合并不丢箱、不乱序。
2. 在结果区新增「装柜步骤」Tab，接入阶段列表和当前阶段高亮。
3. 复用现有 2D/3D 视图能力显示阶段范围，避免把装柜逻辑写死在 UI 组件中。
4. 增加 Playwright 覆盖：计算后能进入装柜步骤页、切换阶段、当前阶段箱体高亮、标签统计可见。
5. 后续单独补打印/导出步骤图，避免第一版同时处理 UI、业务分组和导出格式导致范围过大。
