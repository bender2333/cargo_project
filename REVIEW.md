...
当前本地验证状态：`npx vitest run src/lib/manualPlacement.test.ts` 通过 40 项；`npx tsc -b` 通过；targeted E2E `npx playwright test e2e/manual-3d.spec.ts --grep "R 与 Shift|浮层|键盘帮助|旋转提示|最大化保留|选中前不显示"` 通过 6 项；`npm run lint`、`npm test`、`npm run build` 通过；完整本地 `npm run test:e2e` 为 78 passed / 1 skipped / 0 failed。远程部署与远程 E2E 结果以 `CHANGELOG.md` 最终记录为准。
