...
### 整改任务 (P1)
- [ ] P1-1: 实现自动/手动共享 packing result 终结流程，并支撑有序的 workSteps。
- [ ] P1-2: 实现基于快照 cargo plan 的手动历史记录原子恢复。
- [ ] P1-3: 确保手动校验逻辑应用全局默认堆叠限制。
- [ ] P1-4: 确保活动自动方案通过统一合规上下文忽略隐藏的手动问题。
- [ ] P1-5: 建立权威诊断投影，并去重 Checklist ID。
- [ ] P1-6: 确保自动映射始终开启确认，且取消动作不更改当前修订。
- [ ] P1-7: 禁止非模板选择的隐式 1kg 重量，除非明确选择了模板默认值。
- [ ] P1-8: 在前端 GET/restore、POST 及服务端进行运行时快照校验，并对齐请求体限制。
- [ ] P1-9: 确保懒加载重试在拒绝后能正确重新创建 loader。

### 整改任务 (P2)
- [ ] P2-1: 禁用旋转时清除陈旧的朝向元数据。
- [ ] P2-2: 将快捷键限制在概览和聚焦手动工作区范围。
- [ ] P2-3: 规范弹窗初始焦点、Tab 陷阱、Escape 和焦点恢复行为。
- [ ] P2-4: 在同步预览解析前限制行/单元格扩展。
- [ ] P2-5: 确保混合朝向测试断言前提条件，且禁止提前返回。
- [ ] P2-6: 强制已放置箱体拥有 depthLayer，契约拒绝缺失项。
- [ ] P2-7: 解释并修复合同哈希不一致及 3D 首像素回退，禁止修改基线/阈值/夹具。
